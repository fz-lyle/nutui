...
function buildChangeLog(callback) {
	...
	componentsPackageJsons.forEach(item => {
		...
		if (version !== syncVersionsData[id]) {
			...
			nutuimd += (content.trim() + '\n')
		}
		syncVersion[id] = version
	})
	// console.log(nutuimd);
	nutuimd = nutuimd.trim()
	if (nutuimd) {
		let nutuiPackagePath = path.join(modulesPath, modulesId, 'package.json')
		nutuiPackage = JSON.parse(fs.readFileSync(nutuiPackagePath).toString())
		let mdpath = path.join(modulesPath, modulesId, 'changelog.md')
		let newVersion = nutuiPackage.version.split('.')
		newVersion[2] = Number(newVersion[2]) + 1
		newVersion = newVersion.join('.')
		nutuiPackage.version = newVersion
		buildDocsChangeLog(nutuimd, newVersion)
		const nutuiChangelog = updateChangelogFile(
			mdpath,
			newVersion,
			nutuimd
		)
		// 修改合并后的 release
		fs.writeFileSync(mdpath, nutuiChangelog)
		// package.json 版本号 +1
		fs.writeFileSync(nutuiPackagePath, JSON.stringify(nutuiPackage, null, 2))
...
function getAllComponentsList(modulesPath) {
	...
	fileLists.forEach(name => {
		if (name === modulesId || name === 'nut-test') {
			return
		}
